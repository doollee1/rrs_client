...
<style type="text/css">
	...
	#comPlusBtn {opacity:30%; background-color:#348FE2; width:22px; height:22px; font-size:12px; line-height:22px; right:45px; top:120px;}
...
$(document).ready(function() {
	...
	function setPeopleGbn(s_data) {
		var $combo = $('<select id="list_num_gbn" name="list_num_gbn" style="min-width:70px;" disabled="disabled"/>');
		    ...
		    $combo.append($('<option/>',{'value':'03'}).text('성인')); //여기는 value 없는 그냥 기본 셋팅 값
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(data.result == "SUCCESS") {
					...
					twinCnt = 0;
					kingCnt = 0;
					alert("동반자 수정이 실패하였습니다.관리자에게 문의 하세요.");
				}
		 	}
		});
	}
	...
	function setEvent() {
		...
		$("#reservationUpdateBtn").on("click", function() {
			...
			if(btnText == "수정진행") {
					...
					success : function(data) {
						if(data.result == "SUCCESS") {
							if(data.prc_sts == "${reservationDetail.PRC_STS}") {
								...
								$("#flight_in, #flight_in_dt, #flight_in_hh, #flight_in_mm, #flight_out, #flight_out_dt, #flight_out_hh, #flight_out_mm").removeClass("readonly");
								$("#room_type, #pick_gbn, #add_r_s_day, #add_r_s_day, #add_r_s_per, #add_r_p_per, #add_r_p_day, #comDelete").removeClass("readonly");
								$("#list_table, #comPlusBtn, #late_check_in, #late_check_out").removeClass("readonly");
								$("#m_person, #g_person, #n_person, #k_person, #i_person, #add_hdng_gbn, #remark").removeClass("readonly");
				...
				var data = {
						...
						, flight_in      : $("#flight_in" ).val()								// 비행출발항공기편
						...
						, flight_out     : $("#flight_out").val()								// 비행도착항공기편
				...
				$.ajax({
					...
					success : function(data) {
						if(data.result == "SUCCESS") {
							...
							dimClose();
							alert("상태값이 변경되어 수정할 수 없습니다. 관리자에게 문의 하세요.");
						}
					}
				});
			}
		...
		/******************************************** 
		 * @Subject : datepicker setting
		 * @Content : [.input-daterange] Class 이벤트
		 * @Since   : 2024.07.11
		 * @Author  : K.J.T 
		 ********************************************/
		...
		$("#calBtn").on("click", function() {
			...
			// 레이트 인아웃 산정을 위함 : 숙박인원 성인기준:(멤버 + 일반 + 성인 + 소아)
			roomPerson = Math.round((Number($("#m_person").val()) + Number($("#g_person").val()) + Number($("#n_person").val()) + Number($("#k_person").val()))/2);
			roomPerson = roomPerson > 0 ? roomPerson : 1;
		...
		$("#chk_out_dt").on("change", function() {
			...
			if(!$("#chk_out_dt").is(":disabled") && $("#g_person").val() > 0){
				...
				$("tr#com_board").each(function (index, item) {
					let td = $(this).children();
					let comListChk = td.eq(2).find('#list_num_gbn option:selected').val(); // 인원구분
					if(td.eq(1).text() == "2" && comListChk == "02"){
						td.remove();
					}
				});
		...
		$("#comAddListBtn").on("click", function() {
			...
			if(comRdChk == "01"){
				...
				$("#m_person").val(temPerson);
				
			}else if(comRdChk == "02"){
				temPerson = Number($("#g_person").val())+1;
				$("#g_person").val(temPerson);
				...
				$("#n_person").val(temPerson);
				
			}else if(comRdChk == "04"){
				temPerson = Number($("#k_person").val())+1;
				$("#k_person").val(temPerson);
				
			}else if(comRdChk == "05"){
				temPerson = Number($("#i_person").val())+1;
				$("#i_person").val(temPerson);
			}
		...
		<div class="tab-content panel rounded-0 p-3 m-0">
			...
			<div class="tab-pane fade active show" id="default-tab-1">
				<div class="input-daterange">
					<div class="form-group row mb-2">
						...
						<div class="col-lg-12">
							<div class="input-group date" >
								<input type="text" id="chk_in_dt" name="chk_in_dt" class="form-control text-center" placeholder="날짜를 선택하세요" disabled="disabled">
					...
					<div class="form-group row mb-2">
						...
						<div class="col-lg-12">
							<div class="input-group date" >
								<input type="text" id="chk_out_dt" name="chk_out_dt" class="form-control text-center" placeholder="날짜를 선택하세요" disabled="disabled">
								<span class="input-group-text input-group-addon"><i class="fa fa-calendar"></i></span>
							</div>
						</div>
					</div>
				...
				<div class="row mb-2">
					...
					<input type="text" id="no_room_chk" name="no_room_chk" class="form-control text-center" value="Check OK"
						   style="border:none;border-right:0px; border-top:0px; boder-left:0px; boder-bottom:0px; width:85px; background: #FFFFFF; opacity:60%; color: green;" readonly></label>
					<div class="col-md-9">
				...
				<div class="row mb-2">
					...
					<div class="col-md-9 inline-flex">
						<select id="flight_in" name="flight_in" class="form-select text-center readonly">
							<option value="" style="font-size: 0.9rem;font-weight:bold;">-선택-</option>
						...
						<select id="flight_in_mm" name="flight_in_mm" class="form-select text-center readonly">
				...
				<div class="row mb-2">
					...
					<div class="col-md-9 inline-flex">
						<select id="flight_out" name="flight_out" class="form-select text-center readonly">
							<option value="" style="font-size: 0.9rem;font-weight:bold;">-선택-</option>
						...
						<select id="flight_out_hh" name="flight_out_hh" class="form-select text-center readonly">
							<option value="" style="font-size: 0.9rem;font-weight:bold;">-선택(시)-</option>
							<c:forEach var="i" begin="0" end="23" step="1">
								<option value="<fmt:formatNumber value="${i}" minIntegerDigits="2" />" style="font-size: 0.9rem;font-weight:bold;">
									<fmt:formatNumber value="${i}" minIntegerDigits="2" />시
						...
						<select id="flight_out_mm" name="flight_out_mm" class="form-select text-center readonly">
							<option value="" style="font-size: 0.9rem;font-weight:bold;">-선택(분)-</option>
							<c:forEach var="i" begin="0" end="60" step="1">
								<option value="<fmt:formatNumber value="${i}" minIntegerDigits="2" />" style="font-size: 0.9rem;font-weight:bold;">
									<fmt:formatNumber value="${i}" minIntegerDigits="2" />분
				...
				<div class="row mb-2">
					...
					<div class="col-sm-9">
						<input id="fligth_image" name="fligth_image" type="text" class="form-control text-center" readonly>
					</div>
				</div>
				...
				<div class="row mb-2">
					...
					<div class="col-sm-9">
						<input id="req_han_nm" name="req_han_nm" type="text" class="form-control text-center" readonly>
					</div>
				</div>
				...
				<div class="row mb-2">
					...
					<div class="col-sm-9">
						<input id="req_eng_nm" name="req_eng_nm" type="text" class="form-control text-center" readonly>
					</div>
				</div>
			</div>
			<!-- END tab-pane -->
			...
			<div class="tab-pane fade" id="default-tab-2">
				<div class="row mb-2">
					...
					<div class="col-md-9 inline-flex">
						<select id="pick_gbn" name="pick_gbn" class="form-select text-center readonly">
				...
				<div class="row mb-2">
					...
					<div class="col-md-9">
						<select id="late_check_in" name="late_check_in" style="text-align: center" class="form-select text-center readonly">
				...
				<div class="row mb-2">
					...
					<div class="col-md-9">
						<select id="late_check_out" name="late_check_out" style="text-align: center" class="form-select text-center readonly">
			...
			<div class="tab-pane fade" id="default-tab-3">
				<button id="comPlusBtn" name="comPlusBtn" type="button" class="btn btn-success btn-icon btn-lg btn-write readonly">
								...
								<div class="total-people-wrap" id="reserve_popup">
									<div class="radioSet">
										...
										<div style="margin-top:5px;">
											<input type="radio" id ="r3" name="comAddradio" value="03"/><label for="r3" style="border-radius:8px;width:40%;">성인</label>
				...
				<div>
					<div class="row mb-2">
						...
						<div class="col-md-9">
							<textarea id="remark" name="remark" class="form-control readonly" rows="2"></textarea>
						</div>
					</div>
					...
					<div class="mb-2">
						<div class="inline-flex calc" id="reserve_cal_box"style="display:flex; justify-content:right; width:75%; margin-left:auto; gap:10px;">
							<button id="calBtn" name="calBtn" type="button" class="btn btn-pink" style="min-width: 9rem; height: 2.5rem;" disabled>가계산</button>
				...
				<div class="mb-15px" style="display:flex; gap:5px;">
					<button id="reservationCancelBtn" type="button" class="btn btn-gray h-45px btn-lg fs-14px" style="flex:1">예약취소</button>			
					<button id="reservationUpdateBtn" type="button" class="btn btn-theme h-45px btn-lg fs-14px" style="flex:1">수정진행</button>		
				</div>
			</div>
			<!-- END tab-pane -->
		</div>
